...
case class Last(finished: Boolean, neighbor: ArrayBuffer[ActorRef])
...
class GossipActor(topology: String) extends Actor 
{
  ...
  def receive = 
  {
    ...
    case RemindSelf =>
      s_value = s_value/2
      w_value = w_value/2
      context.system.scheduler.scheduleOnce(1000 milliseconds, neighbor(Random.nextInt(neighbor.length)), PushSum(s_value, w_value))
      context.system.scheduler.scheduleOnce(1000 milliseconds, self, RemindSelf)
    ...
    case Remind =>
      ...
      {
        neighbor(Random.nextInt(neighbor.length)) ! Gossip
...
class Master(numNodes: Int, topology: String, algorithm: String, gap: Int) extends Actor 
{
  ...
  var noNbrCount, noFailNode, finish = 0
  ...
  def receive = 
  {
    ...
    case End =>
      ...
      if (finish == n) 
      {
        ...
        println("Number of Failed Nodes: " + noFailNode)
    ...
    case Fail =>
      context.system.scheduler.scheduleOnce(1 milliseconds, a(Random.nextInt(n)), ShutDown)
      context.system.scheduler.scheduleOnce(gap milliseconds, self, Fail)      
      
    case Last(finished, neighbor) =>
      noFailNode = noFailNode + 1
      var pos = n + noFailNode - 1
      a += context.actorOf(Props(new GossipActor(topology)), name = "allActors" + pos)
      a(pos) ! SetNeighbor(neighbor)
      for (ac <- neighbor) {
        ac ! AddNum(a(pos))
      }
      if (!finished && algorithm == "gossip")
        self ! End
  }
}
...
object project2bonus 
{
  ...
  def gossip(numNodes: Int, topology: String, algorithm: String, gap: Int) 
  {
      val system = ActorSystem("GossipSimulator")
      val master = system.actorOf(Props(new Master(numNodes, topology, algorithm, gap)), name = "master")
      master ! START
  }
  ...
  def main(args: Array[String]) 
  {
	  while(flag!=1)
	  {
		  println("Enter your specification(Format: <Number of Nodes> <Topology> <Algorithm> <Gap of Failure in ms(100 by default)>)")
		  var inputstring = Console.readLine()
		  val inputarr = inputstring.split(" ")
		  if(inputarr.length == 3)
		  {
			  flag = 1
			  gossip(numNodes = inputarr(0).toInt, topology = inputarr(1), algorithm = inputarr(2), gap = 100)
		  }
		  else if(inputarr.length == 4)
		  {
			  flag = 1
			  gossip(numNodes = inputarr(0).toInt, topology = inputarr(1), algorithm = inputarr(2), gap = inputarr(3).toInt) 
		  }
		  else
		  {
